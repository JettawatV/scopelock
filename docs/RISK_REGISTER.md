...
| R-10 | Python or dependency drift makes setup non-reproducible. | Local tests pass on one machine but fail for reviewers or deployment. | `pyproject.toml` requires Python 3.13, package discovery is limited to active code, and `uv.lock` pins 143 packages. | Closed 2026-08-28: `.venv313` uses Python 3.13.14 and ADK 2.8.0; the current 96-test suite passes. | Day 0 | CLOSED |
