...
| R-11 | Frontend work starts before the agent and deterministic workflow gates pass, or grows beyond the thin review workflow. | P0 reliability drops and hackathon time is lost. | The 203-test agent gate and reviewed eval gates passed before the user explicitly unlocked the thin Day 15 UI. Gmail event activation remains held by Day 14. | The UI stays limited to review, evidence, and policy-checked commands; Day 14 passes before `users.watch` is enabled; no forbidden feature appears in the release diff. | All days | CONTROLLED |
| R-16 | Browser-delivered UI exposes the operator key or unsafe client content through storage, logs, or script injection. | An attacker could invoke protected review or commercial endpoints as the operator. | The operator key remains in React memory only; the UI uses same-origin requests, React text escaping, no third-party scripts, no raw email bodies, and a restrictive same-origin CSP. | Source/static-output scans, authenticated endpoint tests, dependency audit, and hosted browser review pass; future third-party scripts require a separate CSP/security review. | Days 15–17 | CONTROLLED |
